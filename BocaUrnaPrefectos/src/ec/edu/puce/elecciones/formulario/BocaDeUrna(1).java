...
public class BocaDeUrna extends JInternalFrame implements ActionListener {
    ...
    public BocaDeUrna(List<Prefecto> prefectos) {
        ...
        btnEliminarVoto = new JButton("Elimnar Voto");
    ...
    private void llenarTabla() {
        model.setRowCount(0);
        for (Prefecto prefecto : prefectos) {
            model.addRow(new Object[]{prefecto.getNombre(), prefecto.getProvincia(), prefecto.getPartido(), prefecto.getVotos()});
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == btnCancelar) {
            ...
            String provincia = (String) comboBoxProvincias.getSelectedItem();
            ...
            if (ciudad == null) {
                ...
                return; // Salir del método sin hacer nada más si no hay ciudad seleccionada
            }

            for (Prefecto prefecto : prefectos) {
                if (prefecto.getNombre().equals(seleccionado)) {
                    prefecto.agregarVoto();
