...
public class ReportePorCiudad extends JInternalFrame {
    ...
    private void llenarTabla() {
        ...
        for (Prefecto prefecto : prefectos) {
            // Verificar que el prefecto sea de la provincia seleccionada
            if (prefecto.getProvincia().equals(provinciaSeleccionada)) {
                // Obtener los votos por ciudad del prefecto
                Map<String, Integer> votosPorCiudad = prefecto.getVotosPorCiudad();
                // Obtener los votos específicos de la ciudad seleccionada
                int votos = votosPorCiudad.getOrDefault(ciudadSeleccionada, 0);
                // Si hay votos registrados para la ciudad seleccionada, agregar a la tabla
                if (votos > 0) {
